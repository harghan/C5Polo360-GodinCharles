...
foreach ($navItems as $ni) {
	...
	echo "<td class='titremenu'>";

	//echo '<li class="' . $ni->classes . '">'; //opens a nav item
	$menu = explode(",",$ni->name);
	//echo '<a href="' . $ni->url . '" target="' . $ni->target . '" class="' . $ni->classes . '">' . $ni->name . '</a>';
	echo '<a href="' . $ni->url . '" target="' . $ni->target . '" class="' . $ni->classes . '">' . $menu[0] . "<br><p class='sousmenu'>". $menu[1] . "</p></a>";

	if ($ni->hasSubmenu) {
		//echo '<ul>'; //opens a dropdown sub-menu
		echo '<tr>'; //opens a dropdown sub-menu
	} else {
		echo '</td>';
		//echo '</li>'; //closes a nav item
		echo str_repeat('</tr></td>', $ni->subDepth);
